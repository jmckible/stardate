...
namespace :export do
  ...
  task :weight, [:user_id] => [:environment] do |t, args|
    ...
    File.open Rails.root.join('tmp', "weight_#{user.id}.xml"), 'w' do |file|
      ...
      user.weights.find_each do |weight|
        file.write "  <Record type=\"HKQuantityTypeIdentifierBodyMass\" sourceName=\"Stardate\" sourceVersion=\"1\" unit=\"lb\" creationDate=\"#{weight.created_at.to_s}\" startDate=\"#{weight.export_start_date}\" endDate=\"#{weight.export_start_date}\" value=\"#{weight.weight}\"/>\n"
      end
