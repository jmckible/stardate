...
describe User do
  ...
  it 'should find stuff on a day' do
    expect(@user.things_during(Date.new(2008, 1, 1)).size).to eq(3) #[items(:today), notes(:today)]
  end
  ...
  it 'should total for the week' do
    expect(@user.total_past_week(Date.new(2008, 1, 1))).to eq(5)
  ...
  it 'should be contained within an amortization' do
    expect(@user.sum_income(Date.new(2007, 12, 27)..Date.new(2007, 12, 29))).to eq(3)
  end
  
  it 'should catch the begining of an amortization' do
    expect(@user.sum_income(Date.new(2007, 12, 20)..Date.new(2007, 12, 24))).to eq(3)
  end
  
  it 'should cath the end of an amortization' do
    expect(@user.sum_income(Date.new(2007, 12, 30)..Date.new(2008, 1, 3))).to eq(2)
  end

  it 'should sum income from a date' do
    expect(@user.sum_income(Date.new(2007, 12, 25))).to eq(1)
  end
  
  it 'should sum from a single date as period' do
    expect(@user.sum_income(Date.new(2007, 12, 25)..Date.new(2007, 12, 25))).to eq(1)
  end
